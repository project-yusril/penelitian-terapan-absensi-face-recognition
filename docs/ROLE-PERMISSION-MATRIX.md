...
| Guard | Route | Fungsi |
|---|---|---|
| `biometric.trusted` | 12 route: permit, check-in/out, sync-offline, enrollment, re-enrollment, embedding, approval enrollment/re-enrollment (API + web) | C-04/H-04: sejak 21 Sep 2026 (revisi ADR-001, keputusan pemilik proyek) `BIOMETRIC_ALLOW_CLIENT_CLAIMS=true` di production membuka route dengan bukti client-attested; tanpa flag tetap `503 TRUSTED_BIOMETRIC_EVIDENCE_REQUIRED`. Trusted verifier tetap di luar scope penelitian ([ADR-001](ADR-001-trusted-biometric-verifier.md) ditolak) |
