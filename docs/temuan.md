...
- [~] **R-02 Bangun load-test runner yang mengukur response latency/failure/timeout secara eksternal.** (Status 12 September 2026: **runner selesai**, eksekusi sesi benchmark belum dilakukan)
  - Runner k6 tersedia di `loadtest/r02_load_test.js` + metodologi `loadtest/README.md`. Read-only (dashboard/jadwal/history — tidak menyentuh check-in/out agar data R-05/R-07 tak terkontaminasi), level simultan 20/30/40 via `-e USERS`, threshold NFR P95 ≤ 2 s dan failure < 1% terpasang sebagai `thresholds` k6, timeout request 5 s tercatat pada metrik `absensi_request_timeouts`, ringkasan avg/p95/max per endpoint via `handleSummary`, raw via `--summary-export`.
  - **Tetap terbuka:** eksekusi tiga level terhadap staging ber-dataset realistis + arsip JSON/stdout + catatan konfigurasi host sebagai bukti (benchmark report/export DoD menunggu sesi ini).
...
- [ ] Query report/export lulus benchmark dataset realistis dan P95 memenuhi NFR. Query-count benchmark konstan sudah ada (M-17 `ExportQueryCountTest`). 12 September 2026: runner eksternal R-02 tersedia — `loadtest/r02_load_test.js` (k6, read-only: dashboard/jadwal/history, level 20/30/40, threshold P95 ≤ 2 s, failure < 1%, metrik timeout terpisah) dengan metodologi di `loadtest/README.md`. Sisa: eksekusi sesi benchmark pada dataset realistis + arsip `--summary-export` sebagai bukti.
