...
Status project saat ini: **memenuhi tujuan penelitian, tetapi belum layak dipakai sebagai sistem absensi produksi atau sumber keputusan akademik resmi**. Sejak 21 September 2026 mutation attendance/enrollment production **diaktifkan** dengan bukti client-attested (keputusan pemilik proyek — revisi ADR-001; sebelumnya fail-closed); matriks kamera Android fisik dan enforcement CI/release environment juga belum memiliki evidence eksternal.

> **Keputusan scope 12 Agustus 2026.** C-04 dan H-04 (trusted biometric verifier server-side) **dinyatakan di luar scope penelitian** dan diterima sebagai **residual risk yang didokumentasikan**, bukan backlog aktif. Rancangannya ditinjau di [ADR-001](ADR-001-trusted-biometric-verifier.md) dan **ditolak** karena biaya integrasi tinggi tanpa nilai untuk penelitian; tujuan penelitian (kelayakan face recognition on-device + geofencing) sudah tercapai dengan matching di perangkat. **Revisi 21 September 2026 (keputusan pemilik proyek):** alur client-attested di production diaktifkan via `BIOMETRIC_ALLOW_CLIENT_CLAIMS=true` untuk demo penelitian; data tetap **client-attested**, bukan bukti forensik, dan C-04/H-04 tetap residual risk. Menaikkan proyek ke tingkat produksi nyata mengharuskan C-04/H-04 dibuka kembali.
...
4. Reliability/performance lapangan dan validitas penelitian: **R-02 selesai (benchmark 21 Sep 2026, p95 < 2 s) dan FAR asli selesai (1.485 pasangan impostor, FAR @ 0.600 = 0%)**; yang masih terbuka: R-03 bagian FRR (butuh data genuine lapangan, protokol siap di `eksperimen/PROTOKOL_EKSPERIMEN.md`) dan R-05 (pengambilan data lapangan asli). R-01 dan R-04 sudah ditutup sehingga metrik geofence dan pemisahan dataset per prodi kini valid.
5. Tiga item milestone tanpa task ID kini dilacak sebagai MS-01 (selesai), MS-02 (policy retention/consent/backup/deletion biometrik), dan MS-03 (browser/device E2E). Dua temuan baru muncul dari pekerjaan sinkronisasi ini: M-23 (limiter API tidak pernah dipasang) dan M-24 (endpoint analisis bocor lintas prodi), keduanya sudah ditutup. 12 September 2026: negative test invarian matriks (Milestone 2) ditutup via `RouteGuardInvariantTest`; runner eksternal R-02 (k6, `loadtest/`) dan scaffold E2E MS-03 (`e2e/`) tersedia. 21 September 2026: benchmark R-02 dieksekusi (server-side, bukti `eksperimen/`) dan FAR asli ditutup; E2E MS-03 dan data genuine FRR (R-03/R-05) tetap terbuka.
...
- **Revisi 21 September 2026 (keputusan pemilik proyek): containment dibuka untuk demo penelitian.** Middleware `RequireTrustedBiometricEvidence` diubah agar `BIOMETRIC_ALLOW_CLIENT_CLAIMS=true` berlaku juga di production; flag diset di server produksi (lihat catatan deploy di [DEPLOYMENT.md](DEPLOYMENT.md)). Endpoint permit, check-in/out, sync-offline, enrollment/re-enrollment, dan approval biometrik kini berfungsi dengan bukti **client-attested** — status trust model tidak berubah: server tetap tidak mengulang matching/liveness, C-04 tetap `[ ]` dan residual risk tetap diterima. Test unit gate diperbarui (flag dihormati di production; tanpa flag tetap 503). Larangan klaim ketahanan terhadap proxy attendance/presentation attack tetap berlaku dan kini lebih penting lagi karena endpoint aktif menerima scalar client.
...
- Status 26 Juli 2026: selesai. Migration `2026_07_26_000001_add_domain_invariant_constraints` menambahkan CHECK constraint untuk koordinat/radius geofence, urutan `jam_selesai > jam_mulai` pada jadwal, urutan tanggal semester/tahun ajaran, `sks`/`total_pertemuan` positif, serta toleransi/geofence/persentase dan urutan threshold SP1-SP2-SP3-DO yang tidak boleh tumpang tindih pada `prodi_settings`. Duplikat mata kuliah dengan `kelas IS NULL` ditutup melalui generated column `kelas_key = COALESCE(kelas,'')` plus unique `(kode_mk, semester_id, kelas_key)`. Composite index ditambahkan mengikuti query utama: `attendances(user_id,tanggal)`, `attendances(jadwal_id,tanggal)`, `attendances(mata_kuliah_id,status)`, `jadwals(hari,status)`, dan `face_embeddings(user_id,status)`. Model `MataKuliah` membuang kolom generated pada insert/update/`replicate()`.
  - **Status 20 Agustus 2026 (RENCANA 2):** mekanisme `kelas_key` DIGANTI. Kolom `kelas`, `kelas_key`, `unique_mk_semester_kelas_key`, dan `dosen_id` dihapus dari `mata_kuliahs` (migration `2026_08_20_000001`); unique baru `unique_mk_semester (kode_mk, semester_id, prodi_id)`. Anti-duplikat kombinasi kelas kini dijaga di tabel `kelas` via `unique_kelas_prodi_semester_tingkat_nama`, dan pivot `mahasiswa_kelas` via `unique_mahasiswa_kelas_user_semester` (riwayat mutasi per semester). Pivot `mahasiswa_mata_kuliah` dihapus (migration `2026_08_20_000002`). Index `jadwals(kelas_id)` & `jadwals(dosen_id)` ditambahkan. Detail ada di rencana2.md dan `CURRENT-ARCHITECTURE.md` → "Struktur Data Akademik (Kelas Master)".
...
### [N-01] Verifikasi wajah dijalankan pada frame ekspresi challenge — FRR lapangan menumpuk

- Bukti: `frontend/lib/features/attendance/presentation/pages/attendance_page.dart` `_analyzeFrame` (sebelum perbaikan): begitu `checkChallenge` lolos (frame ke-3 challenge berturut-turut), frame yang *sedang* dievaluasi — wajah senyum/menoleh/angguk/mata terpejam — langsung dipakai `_verifyFace`. Embedding referensi (server) berasal dari capture final enrollment yang diwajibkan frontal + mata terbuka (`enrollment_page.dart`, kontinuitas identitas), sehingga perbandingan melibatkan dua domain pose berbeda.
- Dampak: jarak genuine sistematis membengkak; threshold live 0.600 berada tepat di bawah lantai impostor hasil studi (min 0.6185 — R-03 FAR), sehingga margin hampir nol. Gejala lapangan: "Verifikasi wajah gagal" berulang padahal wajah pemilik akun — FRR tinggi yang belum terukur karena R-03 bagian genuine masih menunggu data lapangan.
- Perbaikan (21 September 2026, dua lapis):
  1. **Gate frame netral pasca-liveness**: setelah challenge lolos, stream kamera dibiarkan jalan; verifikasi menunggu frame berikutnya yang memenuhi `isFaceFacingFront && areEyesOpen` (ambang identik dengan syarat capture final enrollment, |eulerX|<30 / |eulerY|<30 / mata >0.5), maksimum 5 detik; timeout mengulang liveness dengan challenge baru. State `_awaitingNeutralFrame` di-reset di semua jalur keluar (gagal verifikasi, error verifikasi, gagal kirim, mock location, app lifecycle, dispose).
  2. **Binding wajah pada liveness (temuan review keamanan)**: saat diskontinuitas wajah (`LivenessContinuity.observe` — wajah keluar frame/berganti tracking), fase tunggu netral ikut dibatalkan. Tanpa ini, foto/orang lain yang frontal bisa diverifikasi dalam jendela 5 detik tanpa pernah lolos challenge — bypass anti-spoofing. Verifikasi kini selalu terikat pada wajah yang sama dengan yang melewati challenge.
...
- Status 11 Agustus 2026 (push): seluruh pekerjaan lokal yang sebelumnya belum ter-commit sudah didorong ke `origin/main` — `5e49bfe` (containment biometrik, lifecycle FCM, harness device/CI, manifest deployment), `b271326` (R-04 dan sinkronisasi dokumentasi), `d46f0b1` (koreksi status L-09), dan `13fc302` (M-23, M-24, MS-01). Seluruhnya fast-forward tanpa divergensi. Dengan demikian sub-blocker "perubahan baru belum di-push/dijalankan" sudah hilang dan workflow `backend-ci`/`frontend-ci` seharusnya terpicu pada revision ini.
- Sisa yang menahan `[X]`: GitHub CLI tetap tidak tersedia di workspace ini, sehingga hasil run remote, production/device-testing environments, secret/variable names, branch protection, dan required checks belum dapat diperiksa atau dibuat dari sini. Status green **belum boleh diklaim hanya karena push berhasil** — push hanya memicu workflow, bukan membuktikan hasilnya. L-09 baru selesai setelah Backend CI + Frontend CI benar-benar green pada revision ini, branch `main` mewajibkan keduanya sebagai required check, environment protected tersedia, dan Android release/device workflow minimal satu kali berhasil.
...
- [x] **R-02 Bangun load-test runner yang mengukur response latency/failure/timeout secara eksternal.** (21 September 2026: **runner + eksekusi benchmark selesai**, bukti diarsipkan)
  - Runner k6 tersedia di `loadtest/r02_load_test.js` (v1, satu akun) dan `loadtest/r02_load_test_v2.js` (v2, multi-akun: satu token per akun mahasiswa untuk menghindari rate limiter `api` 60 req/menit/user M-23) + metodologi `loadtest/README.md`. Read-only (dashboard/jadwal/history — tidak menyentuh check-in/out agar data R-05/R-07 tak terkontaminasi), level simultan 20/30/40 via `-e USERS`, threshold NFR P95 ≤ 2 s dan failure < 1% terpasang sebagai `thresholds` k6, timeout request 5 s tercatat pada metrik `absensi_request_timeouts`, ringkasan avg/p95/max per endpoint via `handleSummary`, raw via `--summary-export`.
  - **Eksekusi 21 September 2026 (~01:00 WIB):** WAF hCDN Hostinger menghalang-i runner eksternal (HTTP 403 "checking your browser" untuk seluruh request non-browser, termasuk `/api/health`), sehingga benchmark dijalankan **dari dalam server** dengan curl-loop (`eksperimen/loadtest_server.sh`): 39 token akun mahasiswa approved (satu token per pengguna), `--resolve` ke 127.0.0.1 agar tidak lewat CDN, GET-only dashboard/jadwal/history, 45 detik per level, 20/30/40 pengguna. Hasil: **p95 seluruh level < 2 detik** (dashboard 611→907→1119 ms; jadwal 305→701→796 ms; history 193→696→604 ms) — target NFR PRD-08 terpenuhi. Failure rate tinggi (0.8–75%) seluruhnya HTTP **429** dari rate limiter `api` per-user (by design, bukan crash): 1 akun/VU beriterasi ±3 req × 8 siklus/menit melampaui kuota 60/menit pada level atas. Log Laravel: **0 error baru**. Arsip: `eksperimen/` (skrip + hasil + `PROTOKOL_EKSPERIMEN.md` §2). Catatan metodologi: apabila ingin failure < 1% tanpa 429, turunkan arrival rate per pengguna di bawah kuota limiter — angka p95 tidak berubah signifikan.
- [~] **R-03 Tetapkan metodologi FAR/FRR dan anti-spoofing yang terpisah.** (21 September 2026: **FAR selesai dari data asli; FRR menunggu pengumpulan data lapangan**)
  - **FAR selesai (data asli, 21 September 2026):** 55 embedding approved mahasiswa (dekripsi AES-256-GCM dilakukan di server; embedding tidak keluar dari server) → **1.485 pasangan lintas-user (impostor)** dihitung dengan Euclidean Distance persis logika produksi `BiometricDuplicateService`. Distribusi: min 0.6185, mean 1.1332, maks 1.5083. **FAR @ θ=0.600 (threshold aktif) = 0.0000% (0/1485)**; sweep θ 0.30–1.20: 0.13% @0.65, 1.01% @0.80, 4.92% @0.90, 16.36% @1.00. Skrip + data: `eksperimen/analisis_far_frr.py`, `eksperimen/impostor_distances_20260921.txt`, `eksperimen/hasil_analisis_far.txt`.
  - **Peringatan dataset:** 400 "genuine" + 400 "impostor" lama di `attendance_logs` adalah **data sintetis** dari seeder `attendance:seed-analysis-data` (Gaussian μ=0.35/μ=1.05) — tidak boleh dilaporkan sebagai hasil eksperimen. FAR asli di atas menggantikannya untuk sisi impostor.
  - **FRR tetap terbuka:** butuh skor genuine capture berulang dari orang yang sama (min 30–50; target 15 subjek × 20 capture) via aplikasi normal — protokol lengkap di `eksperimen/PROTOKOL_EKSPERIMEN.md`. `analisis_far_frr.py` otomatis menghitung FRR + EER begitu `genuine_distances.csv` tersedia.
  - Anti-spoofing/PAD tetap terpisah dan belum diuji (protokol uji terbatas foto/video replay juga di dokumen yang sama).
...
- [~] **R-05 Jalankan pengambilan data lapangan serta arsipkan raw evidence.** (Status 20 Agustus 2026: **infra & dataset awal siap**, pengambilan data lapangan asli tetap terbuka)
  - Infra lengkap sejak FIX-LOG-002/003: `TestModeController` (web + `Api\Admin\TestModeController`), `AnalysisController::faceVerification` + `sweepFarFrr`, dan pencatatan label via header `X-Test-Label`/`metadata.label` pada `AttendanceController::buildResearchMetadata`/`logAttempt`.
  - 20 Agustus 2026: command baru `php artisan attendance:seed-analysis-data` mengisi **920 log uji** (genuine=400 μ≈0.35, impostor=400 μ≈1.05, checkin_failed=20, geofence_valid=40, uji simultan=60) sehingga halaman `/analysis` & endpoint `face-verification` tidak lagi kosong (EER 0.75% @ θ optimal 0.6). `test_mode_enabled=1`.
  - Regression: `SeedAnalysisDataTest` (3 test: isi data, idempotensi, `--force`) — 232/232 test PASS.
  - **Tetap terbuka:** data lapangan asli (min 30–50 genuine & impostor, orang hidup berbeda untuk impostor, artefak mentah diaudit) masih harus dikumpulkan sesuai rekomendasi validitas di atas.
...
- [x] Tambahkan negative tests untuk setiap endpoint sensitif. Negative test lintas role/prodi sudah ada untuk user/role (C-02), approval Kaprodi (C-03), workflow SP (C-07), monitoring/dashboard/report/setting (H-21), analisis penelitian (MS-01), dan permit attendance; checklist kelas serangan ada di MS-01. **Invarian matriks selesai 12 September 2026**: `tests/Feature/RouteGuardInvariantTest.php` membaca tabel route dari Router yang ter-boot lalu menggagalkan suite bila ada route non-publik tanpa `role:`/`web.role:` (allowlist eksplisit untuk 27 route publik/user-owned dengan justifikasi), memvalidasi guard hanya mereferensikan 8 role canonical, dan menyertakan sanity check anti-lolos-kosong. Terverifikasi menangkap pelanggaran: route probe tanpa guard yang disisipkan sementara membuat test FAIL dengan daftar route bocor.

### Milestone 3: Attendance Integrity

- [x] C-05, C-06, H-11, H-12, H-13, H-14, dan H-15 selesai.
- [ ] C-04 dan H-16. **C-04 di luar scope penelitian (residual risk diterima, [ADR-001](ADR-001-trusted-biometric-verifier.md) ditolak)**; sejak 21 Sep 2026 production menerima bukti client-attested via flag (revisi ADR-001) — tanpa flag gate tetap 503. H-16 menunggu physical-device matrix Android.
...
- [ ] Lakukan load test dan penelitian hanya setelah flow production stabil. (R-02 benchmark selesai 21 September 2026 — p95 < 2 s pada level 20/30/40; sisa R-03 FRR + R-05 data lapangan.)
...
| MS-03 | M5 | Browser/device E2E enam role dashboard + mobile | Butuh browser runtime dan perangkat fisik; beririsan dengan H-16 (matriks Android) dan L-07 (scan axe/Lighthouse). 12 September 2026: scaffold Playwright tersedia di `e2e/` (enam role: login → dashboard → logout; skip otomatis bila kredensial env tak diset) — eksekusi + arsip HTML report masih terbuka |
...
- [~] Backend berhasil bootstrap, migrate, queue, schedule, dan test pada platform production yang terdokumentasi. Bootstrap, migration, dan test terverifikasi pada PHP 8.3.30 dengan MySQL 8.0.30 (`migrate`, `migrate:rollback`, `migrate:fresh`); manifest Nginx/systemd untuk queue dan scheduler tersedia di `deploy/`. **21 September 2026: scheduler production AKTIF dan terbukti** — cron `schedule:run` per menit di host live, hambatan `proc_open` pada `disable_functions` Hostinger diperbaiki, `schedule:test` DONE, log cron `DONE` per menit, dan bug `$totalSent` pada reminder ditemukan-diperbaiki-dideploy (bukti scheduler benar-benar mengeksekusi task). Sisa: pengamatan jangka panjang queue worker (bagian dari L-09).
- [x] Authorization matrix memiliki negative tests lintas role dan lintas prodi. Matriks canonical tersedia (MS-01, [ROLE-PERMISSION-MATRIX.md](ROLE-PERMISSION-MATRIX.md)) beserta checklist kelas serangan, dan seluruh kelas pada checklist tersebut sudah memiliki negative test (C-02, C-03, C-07, H-21, M-24, H-19, M-21/M-23). 12 September 2026: matriks kini ditegakkan sebagai invarian oleh `RouteGuardInvariantTest` (membaca tabel route dari Router; route non-publik tanpa guard role menggagalkan suite; terbukti menangkap route probe tanpa guard).
...
- [x] Query report/export lulus benchmark dataset realistis dan P95 memenuhi NFR. Query-count benchmark konstan sudah ada (M-17 `ExportQueryCountTest`). 12 September 2026: runner eksternal R-02 tersedia — `loadtest/r02_load_test.js` (k6, read-only: dashboard/jadwal/history, level 20/30/40, threshold P95 ≤ 2 s, failure < 1%, metrik timeout terpisah) dengan metodologi di `loadtest/README.md`. **21 September 2026: eksekusi benchmark selesai dari dalam server (WAF hCDN memblokir runner eksternal) — p95 seluruh level 20/30/40 < 2 detik, 0 error Laravel; bukti diarsipkan di `eksperimen/` (detail di R-02).**
...
- [~] Data penelitian dikumpulkan ulang dari pipeline yang metriknya valid dan dapat diaudit. R-01 dan R-04 selesai sehingga metrik geofence dan pemisahan dataset per prodi sudah valid. 20 Agustus 2026: pipeline R-05/R-07 kini **terisi dataset awal** (seeder `attendance:seed-analysis-data`: 400 genuine + 400 impostor + uji simultan 60) sehingga endpoint analisis & halaman web tidak lagi kosong. **21 September 2026: FAR asli selesai dari 1.485 pasangan impostor (55 embedding approved; FAR @ θ=0.600 = 0.0000%) — data seeder dinyatakan sintetis dan tidak valid untuk laporan; benchmark R-02 selesai (p95 < 2 s); yang masih terbuka: data genuine lapangan untuk FRR + uji anti-spoofing terbatas (protokol `eksperimen/PROTOKOL_EKSPERIMEN.md`).**
