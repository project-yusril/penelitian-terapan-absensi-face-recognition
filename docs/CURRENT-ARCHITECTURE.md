...
Permit mencegah request tanpa preauthorization, wrong binding, dan replay. Permit belum membuat koordinat, face distance, atau liveness result menjadi bukti yang diverifikasi independen oleh server. `RequireTrustedBiometricEvidence` tetap dipasang pada permit, attendance online/offline, enrollment/re-enrollment, reference embedding, serta approval biometrik, tetapi sejak 21 September 2026 (keputusan pemilik proyek — revisi ADR-001) `BIOMETRIC_ALLOW_CLIENT_CLAIMS=true` berlaku juga di production sehingga endpoint tersebut aktif dengan bukti **client-attested**; tanpa flag, gate tetap `503 TRUSTED_BIOMETRIC_EVIDENCE_REQUIRED`.

Data legacy/test bersifat **client-attested**, bukan bukti forensik — begitu pula data attendance production saat ini. Trusted verifier server-side (C-04/H-04) **di luar scope penelitian**: rancangannya ditinjau di [ADR-001](ADR-001-trusted-biometric-verifier.md) dan **ditolak**, sehingga face matching/liveness tetap on-device. Pembukaan alur produksi adalah keputusan eksplisit pemilik proyek untuk demo penelitian dan diterima sebagai residual risk, bukan pekerjaan tertunda. C-04/H-04 dan batas klaim lengkap ada di [THREAT-MODEL-ATTENDANCE.md](THREAT-MODEL-ATTENDANCE.md), [ADR-001](ADR-001-trusted-biometric-verifier.md), dan [temuan.md](temuan.md).
...
- **Gate frame netral pasca-liveness (N-01, 21 September 2026):** verifikasi absensi mobile tidak lagi memakai frame ekspresi challenge. Setelah challenge liveness lolos, `AttendancePage` menunggu frame berikutnya yang frontal + mata terbuka (ambang identik dengan capture final enrollment) — maksimum 5 detik, timeout mengulang liveness. Fase tunggu dibatalkan saat diskontinuitas wajah (`LivenessContinuity`), sehingga wajah yang diverifikasi selalu wajah yang sama dengan yang melewati challenge (binding anti-spoofing). Detail: [temuan.md N-01](temuan.md#n-01-verifikasi-wajah-dijalankan-pada-frame-ekspresi-challenge--frr-lapangan-menumpuk).
...
- **Scheduler production aktif (21 September 2026):** cron `schedule:run` per menit di host live; hambatan `proc_open` pada `disable_functions` Hostinger diperbaiki; `attendance:auto-close`/`mark-absent`/outbox/reminder terverifikasi mengeksekusi (termasuk perbaikan bug `$totalSent` pada reminder). Detail: DEPLOYMENT.md.
- Sistem **memenuhi tujuan penelitian** (face recognition on-device + geofencing terbukti), tetapi **belum production-ready**: C-04/H-04 sengaja di luar scope penelitian ([ADR-001](ADR-001-trusted-biometric-verifier.md) ditolak) dan evidence release L-09/H-16 masih terbuka. Alur attendance client-attested **aktif di production sejak 21 September 2026** (revisi ADR-001, keputusan pemilik proyek) — bukti tetap bukan verifier server-side; sistem tidak boleh diklaim tahan proxy attendance/presentation attack.
