...
| Migrasi built-in Kotlin (N-02) | **Selesai 21 Sep 2026 (sisa 1)** — app gradle bermigrasi, 5 plugin di-upgrade; sisa: `safe_device` masih KGP, `android.builtInKotlin` tetap `false` sampai upstream bermigrasi | [temuan.md](temuan.md#n-02-plugin-android-memakai-kotlin-gradle-plugin-kgp--build-gagal-di-flutter-mendatang-agp-9), [DEPLOYMENT.md](DEPLOYMENT.md) |
...
**Pembaruan terakhir:** 21 September 2026 (dua perbaikan mobile + migrasi build: **N-01** verifikasi wajah absensi menunggu frame netral pasca-liveness dengan binding wajah—challenge, **N-02** app gradle bermigrasi ke built-in Kotlin + upgrade 5 plugin — warning KGP tinggal `safe_device` saja, tercatat sebagai sisa; APK debug 4 varian di-rebuild memuat keduanya; detail di [temuan.md N-01](temuan.md#n-01-verifikasi-wajah-dijalankan-pada-frame-ekspresi-challenge--frr-lapangan-menumpuk) dan [N-02](temuan.md#n-02-plugin-android-memakai-kotlin-gradle-plugin-kgp--build-gagal-di-flutter-mendatang-agp-9). Sebelumnya 14 September 2026: deploy backend live via SSH: `face_threshold` 0.600 semua prodi, 11 file backend + build Vite baru, perbaikan EXIF embedding — kredensial & pola deploy di [`ssh.md`](../ssh.md), riwayat di [DEPLOYMENT.md](DEPLOYMENT.md#riwayat-deploy-via-ssh). Analisis 19 face embeddings terverifikasi sehat: 14 pending 14 September tidak perlu re-enrollment. Backend penelitian live di `https://absensi.yusrilekamahendra.com` — health 200, dashboard login aktif, mobile sudah diuji konek dari perangkat fisik; detail di [DEPLOYMENT.md](DEPLOYMENT.md#backend-production-live). Verifikasi tooling terakhir lihat [temuan.md](temuan.md#hasil-verifikasi-tooling): `php artisan test` 235/235, `flutter test` 207, `flutter analyze` bersih, `npm run build` lulus).
