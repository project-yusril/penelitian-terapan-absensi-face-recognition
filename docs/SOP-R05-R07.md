...
| k6/runner eksternal mendapat `403` "Checking your browser" dari `absensi.yusrilekamahendra.com` | WAF hCDN Hostinger memblokir semua request non-browser dari luar (terverifikasi 21 Sep 2026, termasuk `/api/health`) | Jalankan benchmark **dari dalam server**: curl-loop dengan `--resolve <domain>:443:127.0.0.1 --insecure` (bypass CDN + force-HTTPS; pola lengkap `eksperimen/loadtest_server.sh`), atau uji ke host lokal/staging. Hasil benchmark 21 Sep 2026 memakai jalur ini — p95 < 2 s semua level |
